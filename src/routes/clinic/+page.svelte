<script>
	...
	let clncRV, clncRC, userLN, userFN, userMN;
	...
	async function fetchTransactions() {
		try {
			...
			data.sort((a, b) => new Date(b.clncDT) - new Date(a.clncDT));
	...
	function handleSearchClinic(event) {
		searchQuery = event.target.value.toLowerCase();
		limitedData = data
			.filter(
				(user) => user.clncBY.toLowerCase().includes(searchQuery)
				//user.bookNM.toLowerCase().includes(searchQuery)
			)
			.slice(0, 6);
	}
	...
	function handleTransact() {
		...
		clncRV = document.getElementById('clncRV').value;
		clncRC = document.getElementById('clncRC').value;

		userLN = document.getElementById('userLN').value;
		userFN = document.getElementById('userFN').value;
		userMN = document.getElementById('userMN').value;

		let clncBY = `${userLN}, ${userFN} ${userMN}`;

		const logRef = doc(db, 'csjd-main', 'data', 'clinic', clncID);
		...
		setDoc(logRef, logData)
			.then(() => {
				...
				handleAction(
					'Successful',
					`User ${selectedUserData.userUN} ${clncRV} ${clncRC}`,
					`${loclLN}, ${loclFN} ${loclMN}`
				);
			})
			.catch((error) => {
...
<div class="flex flex-col w-screen h-screen overflow-x-hidden bg-border">
							...
							<li>
								...
								<div class="menu-item-collapse">
									<div class="min-h-0">
										...
										<a
											href="/clinic"
											class="menu-item menu-active ml-6">Clinic Management</a>
									</div>
								</div>
							</li>
						</ul>
	...
	{#if modlST}
		<div
			...
			<div class="flex flex-col">
				<h1 class="text-xl font-semibold">Clinic History</h1>
				<p class="text-xs">Below is a log of all clinic visits.</p>
			</div>
			<div class="divider my-0" />
			...
			<div class="flex w-full overflow-x-auto">
				<table class="table table-hover table-compact">
					...
					<tbody>
						{#each limitedData as item (item.clncID)}
			...
			{#if isSelecting && !isCreating}
				...
				<div class="divider my-0" />
				<div class="flex flex-col">
					<h1 class="text-xl font-semibold">Clinic Information</h1>
					<p class="text-xs">Provide the details for this visit.</p>
				</div>
				<div class="form-group flex lg:flex-row">
					<div class="form-field w-full">
						<label
							for="clncRV"
							class="form-label">Reason for Visit</label>
						<input
							id="clncRV"
							class="input max-w-full"
							bind:value={clncRV} />
					...
					<div class="form-field w-full">
						<label
							for="clncRC"
							class="form-label">Nurse's Recommendation</label>
						<input
							id="clncRC"
							class="input max-w-full"
							bind:value={clncRC} />
