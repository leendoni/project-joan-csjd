<script>
	...
	let tranPP, tranAM, userLN, userFN, userMN;
	...
	async function fetchTransactions() {
		try {
			...
			data.sort((a, b) => new Date(b.tranDT) - new Date(a.tranDT));
	...
	function handleTransact() {
		...
		tranPP = document.getElementById('tranPP').value;
		tranAM = document.getElementById('tranAM').value;

		userLN = document.getElementById('userLN').value;
		userFN = document.getElementById('userFN').value;
		userMN = document.getElementById('userMN').value;

		let tranBY = `${userLN}, ${userFN} ${userMN}`;

		const logRef = doc(db, 'csjd-main', 'data', 'transactions', tranID);
		...
		setDoc(logRef, logData)
			.then(() => {
				...
				handleAction(
					'Successful',
					`User ${selectedUserData.userUN} paid for ${tranPP}`,
					`${loclLN}, ${loclFN} ${loclMN}`
				);
			})
			.catch((error) => {
...
<div class="flex flex-col w-screen h-screen overflow-x-hidden bg-border">
	...
	{#if modlST}
		<div
			...
			<div class="flex flex-col">
				<h1 class="text-xl font-semibold">Transactions Masterlist</h1>
				<p class="text-xs">Below is a list of all logged transactions.</p>
			</div>
			<div class="divider my-0" />
			...
			<div class="flex w-full overflow-x-auto">
				<table class="table table-hover table-compact">
					...
					<tbody>
						{#each limitedData as item (item.tranID)}
			...
			{#if isSelecting && !isCreating}
				...
				<div class="divider my-0" />
				<div class="flex flex-col">
					<h1 class="text-xl font-semibold">Transaction Information</h1>
				...
				<button
					class="btn btn-outline-success"
					on:click={() => handleTransact()}>Save Transaction</button>
			{/if}
		</div>
	{:else if !modlST}
		<div
