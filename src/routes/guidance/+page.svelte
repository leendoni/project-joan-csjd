<script>
	...
	let guidVL, guidOF, userLN, userFN, userMN;
	...
	async function fetchTransactions() {
		try {
			...
			data.sort((a, b) => new Date(b.guidDT) - new Date(a.guidDT));
	...
	function handleSearchViolation(event) {
		searchQuery = event.target.value.toLowerCase();
		limitedData = data
			.filter(
				(user) =>
					user.guidBY.toLowerCase().includes(searchQuery) ||
					user.guidVL.toLowerCase().includes(searchQuery)
			)
			.slice(0, 6);
	}
	...
	function handleTransact() {
		...
		guidVL = document.getElementById('guidVL').value;
		guidOF = document.getElementById('guidOF').value;

		userLN = document.getElementById('userLN').value;
		userFN = document.getElementById('userFN').value;
		userMN = document.getElementById('userMN').value;

		let guidBY = `${userLN}, ${userFN} ${userMN}`;

		const logRef = doc(db, 'csjd-main', 'data', 'guidance', guidID);
		...
		setDoc(logRef, logData)
			.then(() => {
				...
				handleAction(
					'Successful',
					`User ${selectedUserData.userUN} paid for ${guidVL}`,
					`${loclLN}, ${loclFN} ${loclMN}`
				);
			})
			.catch((error) => {
...
<div class="flex flex-col w-screen h-screen overflow-x-hidden bg-border">
							...
							<li>
								...
								<div class="menu-item-collapse">
									<div class="min-h-0">
										...
										<a
											href="/guidance"
											class="menu-item menu-active ml-6">Violations & Sanctions</a>
									</div>
								</div>
							</li>
							<li>
	...
	{#if modlST}
		<div
			...
			<div class="flex flex-col">
				<h1 class="text-xl font-semibold">Violation Masterlist</h1>
				<p class="text-xs">Below is a list of all logged violations.</p>
			</div>
			<div class="divider my-0" />
			<div class="flex flex-row justify-between">
				<div class="flex flex-row gap-2">
					<input
						class="input"
						placeholder="Search..."
						on:input={handleSearchViolation} />
				</div>
				<div class="flex flex-row gap-2">
					<button
			...
			<div class="flex w-full overflow-x-auto">
				<table class="table table-hover table-compact">
					...
					<tbody>
						{#each limitedData as item (item.guidID)}
			...
			{#if isSelecting && !isCreating}
				...
				<div class="divider my-0" />
				<div class="flex flex-col">
					<h1 class="text-xl font-semibold">Violation Information</h1>
				...
				<div class="form-group flex lg:flex-row">
					<div class="form-field w-full">
						<label
							for="guidVL"
							class="form-label">Violation Details</label>
						<input
							id="guidVL"
							class="input max-w-full"
							bind:value={guidVL} />
					...
					<div class="form-field w-full">
						<label
							for="guidOF"
							class="form-label">Xth Number of Offense</label>
						<input
							id="guidOF"
							type="number"
							class="input max-w-full"
