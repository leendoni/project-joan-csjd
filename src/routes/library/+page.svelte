<script>
	...
	let bookAC, bookNM, userLN, userFN, userMN;
	...
	async function fetchTransactions() {
		try {
			...
			data.sort((a, b) => new Date(b.bookDT) - new Date(a.bookDT));
	...
	function handleSearchLibrary(event) {
		searchQuery = event.target.value.toLowerCase();
		limitedData = data
			.filter(
				(user) =>
					user.bookBY.toLowerCase().includes(searchQuery) ||
	...
	function handleTransact() {
		...
		bookAC = document.getElementById('bookAC').value;
		bookNM = document.getElementById('bookNM').value;

		userLN = document.getElementById('userLN').value;
		userFN = document.getElementById('userFN').value;
		userMN = document.getElementById('userMN').value;

		let bookBY = `${userLN}, ${userFN} ${userMN}`;

		const logRef = doc(db, 'csjd-main', 'data', 'library', bookID);
		...
		setDoc(logRef, logData)
			.then(() => {
				...
				handleAction(
					'Successful',
					`User ${selectedUserData.userUN} ${bookAC} ${bookNM}`,
					`${loclLN}, ${loclFN} ${loclMN}`
				);
			})
			.catch((error) => {
...
<div class="flex flex-col w-screen h-screen overflow-x-hidden bg-border">
							...
							<li>
								...
								<div class="menu-item-collapse">
									<div class="min-h-0">
										<a
											href="/library"
											class="menu-item menu-active ml-6">Library Management</a>
										<a
											href="/clinic"
	...
	{#if modlST}
		<div
			...
			<div class="flex flex-col">
				<h1 class="text-xl font-semibold">Library History</h1>
				<p class="text-xs">Below is a list of book histories.</p>
			</div>
			<div class="divider my-0" />
			...
			<div class="flex w-full overflow-x-auto">
				<table class="table table-hover table-compact">
					...
					<tbody>
						{#each limitedData as item (item.bookID)}
			...
			{#if isSelecting && !isCreating}
				...
				<div class="divider my-0" />
				<div class="flex flex-col">
					<h1 class="text-xl font-semibold">Library Information</h1>
